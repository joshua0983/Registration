...
public class Student implements Comparable{
    ...
    public int compareTo(Object s){
        ...
        if(this.gradYear > student.gradYear){
        ...
        else if(this.gradYear < student.gradYear){
            return -1;
        }
        else{
            ...
            else if(this.drawNumber < student.drawNumber){
                return -1;
            }
        }
        return 0;
    ...
    public boolean isRegisteredFor(Course maybe){
        for(int i=0;i<schedule.size();i++){
            if(schedule.get(i).dept.equals(maybe.dept) && schedule.get(i).courseNum == maybe.courseNum){
                return true;
            }
        }
        return false;
    ...
    public double totalRegisteredCredits(){
        double credits = 0;
        for(int i=0; i<schedule.size(); i++){
            credits += schedule.get(i).credits;
        }
        return credits;
    }
    ...
    public boolean hasAConflict(Course maybe){
        for(int i=0; i<schedule.size(); i++){
            if(schedule.get(i).conflictsWith(maybe)){
                return true;
            }
        }
        return false;
